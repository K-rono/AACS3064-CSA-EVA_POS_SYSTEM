...
.DATA
		...
		; MODIFY
        STR_NOTHING                     DB ,"    ","NOTHING IN CART","$"
        STR_SELECT                      DB ,"    ","SELECT A BOOK: ","$"
        ...
        STR_MODIFY7                     DB ,15 DUP (" "),"(-) LEFT ARROW KEY ",179,179," RIGHT ARROW KEY (+)","$"
        ; PAYMENT
        ...
        STR_PAYMENT_FORMAT2 DB ,"  ",179,"    ","$"    ; 2 NUMBER AND BOOK
        STR_PAYMENT_FORMAT3 DB ,"    ",179,"   ","$" ; 3 BOOK AND PRICE
        STR_PAYMENT_FORMAT4 DB ,"   ",179,"    ","$"; 4 PRICE AND QTY
        STR_PAYMENT_FORMAT5 DB ,"    ",179,"$"
        STR_PAYMENT_FORMAT7 DB ,179,"$"
        STR_PAYMENT_FORMAT6 DB," ",179,"    ","$"
        ...
        STR_CHECKOUT_COMPLETE   DB 0DH,0AH,0DH,0AH,"    ","TRANSACTION COMPLETE.",'$'
...
PRINT_WHOLE_NUM_F MACRO WHOLE_NUM
    XOR DX,DX
...
SELECT_TO_MODIFY_PROC PROC  
	...
	CONTINUE_MODIFY:
        ...
        DELETE_ORDER:
			...
			MOV DI,SI			; DI = 0002	; DI = MODIFIED CART LIST
			ADD SI,4
			MOV CX,NUMBER_CART	; NUMBER CART CONTAIN 2
			DEC CX
        DELETE_ORDER_LOOP:
...
NO_ROUND3:
    ...
    POP BX
    INC AX
    PUSH DX     ;FLOAT WITHOUT CARRY
    PUSH AX

    NOT_CARRY:
    MOV AX,TOTAL_WHOLE_NUM
    CMP AX,300
    JGE CONT_10_DISCOUNT

    POP AX
    POP BX
    PUSH AX
    JMP CHECK_MEMBER_DISCOUNT
...
;SUBTRACT 10% DISCOUNT
    ...
    PUSH AX
    ;CHECK BORROW
    CMP BX,DISCOUNT_10_FLOAT ;  BX - DISCOUNT_10_FLOAT
    JG NO_BORROW1
...
CHECK_MEMBER_DISCOUNT:
    ...
    POP BX
    POP AX      ;GET WHOLE_NUM CALC
    PUSH BX

    JNE CONT_MEMBER_DISCOUNT

    POP BX
    JMP FINISH_SUM

CONT_MEMBER_DISCOUNT:
;SUBTRACT 8% MEMBER DISCOUNT
    ...
    XOR DX,DX
    MOV AX,BX
    MOV BX,100      ;WHEN CARRY OCCURED DURING SST ADDITION, REMOVE THE HUNDREDS
    DIV BX
    MOV BX,DX
...
FINISH_SUM:
    ...
    PRINT_WHOLE_NUM_F FINAL_BILL_WHOLE_NUM
    PRINT_FLOAT_F FINAL_BILL_FLOAT

    JMP AFTER_CALC_TOTAL
CALC_TOTAL ENDP
